...
- **Per-gene curation (task 48):** representatives are chosen by a deterministic, rank-aware breadth-first traversal of each candidate's GenBank taxonomy lineage — spend the budget evenly across the highest ranks first (class before order, order before family, family before genus), recursing until it bottoms out at one record per genus (longest translation wins, accession as tie-break). This replaces task 29's genus-bucket-in-file-order selection, which in practice yielded consecutive RefSeq accessions rather than phylogenetic spread. The budget itself is **per-origin, and for `animal_mt` is no longer a single number**: `plant_pt` and `plant_mt` keep 5–10 representatives per gene (the marginal-gain argument in task 29 — miniprot tolerates divergence, and a 50th representative buys little next to the first five spanning the clade — still holds for the angiosperms, which are conserved and densely sampled; [§9 item 11](07-open-questions.md#9-fine-tuning-post-prototype-benchmarking)). `animal_mt` instead carries **two disjoint strata per gene: 1,000 representatives spanning `Arthropoda`, plus 100 spanning the rest of Metazoa** as a floor for non-arthropod animal submissions. Metazoan mitochondrial proteins are far more divergent and far more unevenly sampled than the angiosperms, and this pipeline's `animal_mt` intake is concentrated in Arthropoda even though its declared scope (`assembly_target: animal_mt`) is not — a near-conspecific aphid assembly measured at only 48–73% protein identity against the old 10-representative pan-metazoan panel because no aphid, and nothing close to one, was in it (task 30, re-diagnosed task 48). A scaling sweep (task 48 §A) showed identity still climbing steeply from 250 to 1,000 representatives, with the practical ceiling set not by miniprot (linear, cheap to ~100,000 queries) but by the candidate GFF `MINIPROT_CDS` stages across a process boundary on every sample. The non-arthropod floor, plus MITOS2's untouched pan-metazoan reference data, is what keeps `animal_mt` meaning all of Metazoa in practice, not just in the enum. A hard minimum (≥3 candidates, applied to the total across strata) excludes a gene from the panel if RefSeq annotation coverage is too thin; the gene still appears in `organelle_gene_sets.json`'s completeness yardstick, just absent from the `.faa` panel. Strata are build-time arguments to `build_proteins.py` (`--stratum ORIGIN:LINEAGE_TERM=BUDGET`, repeatable, applied in declaration order with mutual exclusion — a record matching an earlier stratum's term is never eligible for a later one), not runtime config: nothing in the pipeline reads them, so they carry no [principle 9](../CONSTITUTION.md) obligation and live in provenance instead.
- **Version pinning:** record per-gene candidate/representative counts, chosen accessions, RefSeq release, the parse method, and — per origin — the strata as invoked (lineage term + budget) and as achieved (candidates seen, representatives selected, distinct genera) in `refs/<version>/proteins/provenance.json`, folded into `manifest.json` by `build_manifest.py`. An under-filled stratum (too few distinct lineages carrying that gene) is recorded, not silently absorbed — budget-vs-selected is what makes that visible.
...
│   ├── animal_mt/<GENE>.faa      # 13 genes, 1,100 reps/gene (1,000 Arthropoda + 100 other Metazoa)
│   ├── plant_pt/<gene>.faa       # ~79 genes, 10 reps/gene
│   └── plant_mt/<gene>.faa       # ~40 genes, mitochondrial CDS only, 10 reps/gene
├── annotate/

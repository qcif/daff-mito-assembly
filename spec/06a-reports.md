...
- **Four reader-perspective tabs, not one tab per pipeline stage:** *Overview*, *Validation*, *Assembly*, *Barcodes*. The pipeline's stage boundaries are an implementation detail; a biosecurity officer asks "did it work, should I trust it, what did I get, which barcodes can I use" — the tabs answer those questions in that order (task 46_report_enhancement.md §4: trustworthiness precedes content, and on a soft-failed sample Validation is already the terminal tab, so it sits where the reader arrives next after Overview). Several stages therefore share a tab, and one stage's output (the coverage gate) is deliberately filed under the question it answers rather than the stage that produced it.
...
|---|---|---|
| **1. Overview** (default) | Derived summary + `samples.csv` (§0) + `NANOPLOT_RAW`/`NANOPLOT_CLEAN` + run provenance | Four blocks, in this order. **(a) Input params and run context** — a compact, visually subordinate strip: sample_id, kingdom, organelle, and any submitter-supplied optional columns from `samples.csv`; the "view all parameters" and "view tool versions" modals; the facility / analyst / analysis-started / analysis-completed / wall-time panel; and the run provenance lines (pipeline commit, reference-bundle version and build date). Kept compact and above Key findings so it orients the reader without pushing Key findings below the fold (task 46_report_enhancement.md §3.1). **(b) Key findings** — a two-column summary table: assembly outcome, coverage, top BLAST hit, gene-feature count, panel-locus extraction count. This is the negative-clarity surface of §6a.1's last bullet and it remains the first thing the eye lands on. **(c) The warnings mirror** — every warning raised on any other tab, each labelled with the tab that owns it. **(d) Sequencing data quality** — pre/post read stats and the filter-yield summary from CHOPPER + FILTLONG, from `metadata.json`'s `read_qc`. Read QC is an *input* property, not a result, which is why it sits here rather than with the analysis tabs: it tells the reader what they submitted and how much of it survived filtering. Link out to NanoPlot's own full report rather than re-rendering its distribution plots. |
| **2. Validation** | `RECRUIT` + `COVERAGE_GATE` + `BLAST_VALIDATE` + `annotation.cds_crosscheck` | "Should I trust it." Everything bearing on the result's trustworthiness, gathered in one place regardless of which stage produced it. **Recruitment + coverage gate:** recruited-read count and total bases, estimated coverage vs the configured HARD MIN / WARN / MAX (§2.1), gate decision (pass / pass-with-warning / subsampled / soft-fail) with the seqtk fraction and seed if applicable, and Flye's own mean coverage on the contig(s) actually selected as the target — labelled by what it was measured over ("recruited reads" vs "assembled organelle"), not by tool name (task 46 §9). On a **pass with warning** (`low_coverage`, [§2.1.3](02-stages.md#213-decision-matrix)) this tab carries a prominent warning that the assembly was produced under the warn floor and that missing genes and fragmented contigs are expected — the reader must not read incompleteness as a negative finding. On plant targets, also the sibling-organelle split of the recruited pool from `sample_status.json` — `target_assigned_bases` vs `sibling_assigned_bases` and the resulting `sibling_organelle_fraction` ([§2.1.5](02-stages.md#215-sibling-organelle-carry-over-in-the-estimate)). Where `coverage_basis` is `total_recruited`, say so: the sibling split was unavailable and the estimate may over-state target depth. **Homology:** top hits per target contig against the kingdom-appropriate RefSeq organelle DB (§4.2) — identity, coverage, subject accession. Explicit note when top-hit identity is below species-threshold — flags novel/underrepresented taxa without pretending to *assign* them (assignment is Taxodactyl's job — see [brief.md §2](brief.md) boundary). **Annotation cross-checks:** where `annotation_summary.json` records `cds_crosscheck` disagreements between miniprot and MITOS2, surface them — two independent methods disagreeing on a gene is a real QC signal — along with `genetic_code_agreement: false`. When a sample soft-failed, **this tab is the terminal content** — no assembly was attempted, and the Assembly and Barcodes tabs must say so rather than render empty tables. |
| **3. Assembly** | `METAFLYE` + `BANDAGE_NG` + `BIN_TARGET` + `ANNOTATE` → `JOIN_ANNOTATION_SCORES` | "What did I get." Assembly statistics (contigs, N50, total bp) and the per-contig breakdown (length, coverage, circularised y/n), the latter alongside a horizontal per-contig mean coverage chart that scales its height with contig count. Polishing is Flye's built-in single-iteration pass — no separate polish section until MEDAKA is un-deferred (§7 Q6). Assembly graph, binning classification (target / secondary / off-target). **The graph is an inline, node-labelled SVG** ([§2 stage 9](02-stages.md#2-stage-detail)), injected into the DOM rather than shown via `<img>` so its per-node tooltips bind — the same treatment the organelle map gets, and the reason stage 9 emits SVG instead of PNG. Hovering a node names the GFA segment (`edge_3`) and lists the contig(s) whose `graph_path` traverses it. It stays a **diagnostic**: node colour carries no binning meaning, because a repeat edge shared between a target and an off-target contig has no single correct bucket — the binning classification is read from the per-contig table, not from the picture. Carries the plastid QC signals of [§3.6](03-organelles.md#36-plastid-quadripartite-canonicalisation): the edge-count reading, the "canonical structure, no taxonomic support" warning when the `path1` substitution was withheld, and `target_source` provenance when it was applied. **No BUSCO** — mock includes it, our spec does not (kingdom-agnostic organelle BUSCO sets are patchy; revisit if needed). **The annotated organelle map lives here too**, as the visual form of the same assembly: inline SVG, clickable/hoverable gene features. tRNA/rRNA features come from MITOS2 on `animal_mt`; plant arms are CDS-only pending [§8 item 3](07-open-questions.md#8-remaining-open-questions) and **must say so** rather than leaving the absence to be inferred ([principle 7](../CONSTITUTION.md)). Confidence detail — `pident`/`qcovhsp`/`bitscore` from `annotation_summary.json`'s `cds_scores` (task 40) — lives behind an "Annotation details" modal, read as two independent axes rendered as separate Completeness/Reference icons: high identity + low coverage is a truncated fragment of a well-referenced gene (Completeness: warning), low identity + high coverage is a complete gene in an under-referenced clade (Reference: an informational marker, **never** a failure/danger icon — task 46 §3.2). A feature with a `null` triplet (no blastp hit against its own gene's panel) renders as an explicit unscored marker on both axes, not as zero confidence. The report may sort or style features by score — that is the only consequence a score is permitted to have (task 40 §5); it never changes which features appear or a sample's status. `status: "annotator_failed"` (task 41) must render distinctly from `ok_cds_only`: both ship a CDS-only annotation, but `ok_cds_only` is the honest provisional state of a target with no annotator configured, while `annotator_failed` is a configured annotator that ran and returned nothing — the `reason` field names the tool and exit code and must be shown, not just the CDS-only feature set. A reader must never mistake a tool failure for a provisional-by-design state. The annotation GFF downloads from here (§7). |
...
- **Renderer inputs beyond `metadata.json`.** The file-shaped artefacts
  ...
  organelle map SVG and the Bandage assembly-graph SVG (both injected
  into the DOM, not via `<img>`, so per-feature and per-node tooltips
  bind), the annotation GFF (base64 `data:` URI), the two NanoPlot HTML
  reports, and

...
| C4 | `plastid_canonicalise.py` — clean-room re-implementation of the algorithm in [§3.6](03-organelles.md#36-plastid-quadripartite-canonicalisation-ptgaul-derived). Upstream reference: [ptGAUL `combine_gfa.py`](../reference-material/ptgaul/combine_gfa.py) (retained under `reference-material/` for provenance only — no licence, source not copied). | shares `wf5/bin-target:<tag>` (invoked by C3 on the plant-cp branch) | [§2 stage 10](#2-stage-detail), plant-cp branch only | Parses Flye's `assembly_graph.gfa`, classifies edges by length + depth (LSC = longest, IR = deepest, SSC = remainder for the 3-edge case), emits `path1.fasta` and `path2.fasta` for the two SSC-orientation isoforms. Handles the 1-edge (resolved circle passthrough) and N≠3 (diagnostic-only) cases explicitly. **Re-implemented in-house** because upstream is unmaintained and unlicensed. | [§3.6](03-organelles.md#36-plastid-quadripartite-canonicalisation-ptgaul-derived) |

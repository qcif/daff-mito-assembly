...
| 9 | `BANDAGE_NG` | BandageNG + C12 ([below](#22-custom-logic-components)) | `assembly_graph.gfa`, `assembly_info.txt` | `<sample_id>.graph.svg` — node-labelled, hover-interactive | — | Diagnostic only; helps human review of tangled assemblies / multi-organism cases. **SVG, not PNG**, so the report can bind per-node tooltips instead of shipping a flat raster ([§6a.2](06a-reports.md) tab 3). **BandageNG's SVG carries no node identity** — its `image` subcommand emits a Qt painter dump (`<desc>Generated with Qt</desc>`): anonymous `<path>` elements, no `id` attributes, no `<text>`, and no `--names`/`--labels` option to add any. Node identity is therefore *round-tripped through colour*: the process writes a `name,color` CSV (header required — BandageNG rejects a headerless file with `didn't contain color`) assigning every GFA segment a unique sentinel `#RRGGBB`, passes it as `--color`, and each node then renders as exactly one path carrying that exact fill. C12 maps fill → segment name, rewrites the sentinel to a neutral display colour, and attaches `data-node` + a `<title>` child so hover shows the segment label (`edge_3`). Sentinel colours are a machine-readable tag only, never a visual channel — nothing downstream may read meaning into them. **Bucket colouring of graph nodes is deliberately not specified:** Flye's segments (`edge_N`) and contigs (`contig_N`) are different namespaces joined many-to-many through `assembly_info.txt`'s `graph_path` column, so a repeat edge traversed by both a target and an off-target contig has no single correct colour — C12 records the traversing contig(s) as `data-contigs` and leaves the ambiguity visible rather than resolving it silently ([principle 7](../CONSTITUTION.md)). Binning is not known at this stage anyway (`BIN_TARGET` is stage 10). |
...
| 12 | `MINIPROT_CDS` (+ `SELECT_GENETIC_CODE` on `animal_mt`) | miniprot (+ C9, [below](#22-custom-logic-components)) | target contigs, **comprehensive** protein panel for the sample's `assembly_target` | `cds.gff` — every protein-coding feature found, `genetic_code.json` — the NCBI translation table used and why | [§4.3](04-reference-data.md#43-protein-panel-and-barcode-selector) | **One broad pass, two consumers.** Protein-to-genome; works on divergent taxa with no same-species reference. The panel is the organelle's full protein-coding complement (13 genes for `animal_mt`, ~79 for `plant_pt`, ~40 for `plant_mt`), not the barcode subset. Output feeds both stage 13 (barcodes) and stage 13a (annotation), so the locus shipped to Taxodactyl is by construction the same object as the locus drawn on the report's gene map ([§8 item 3](07-open-questions.md#8-remaining-open-questions), [rule 18](../CONSTITUTION.md)). Never de-duplicate by gene name — a plastid gene inside the inverted repeat genuinely occurs twice ([§3.6](03-organelles.md#36-plastid-quadripartite-canonicalisation)). **Always runs miniprot's `-T` explicitly** (task 38 §1) — `plant_pt` under table 11, `plant_mt` under table 1, `animal_mt` under a per-sample selection (below). Never translates under miniprot's table-1 default, which reads every internal Trp as a premature stop on `animal_mt` and truncates or drops genes. **`animal_mt` clade trial:** table has no single correct value (vertebrate 2 vs. invertebrate 5 disagree on AGA/AGG/TGA — [§3.3](03-organelles.md#33-specific-issues-and-decisions)), so `MINIPROT_CDS` runs miniprot once per configured table into `candidates/cds.<table>.gff`, and the separate `SELECT_GENETIC_CODE` process ([`select_genetic_code.py`](#22-custom-logic-components), C9) picks the winner by summed best-per-gene alignment score, tie-broken on distinct gene count. Split into two processes because the miniprot biocontainer has no Python to run C9 in (task 38 §2). `plant_pt`/`plant_mt` are single-valued and skip `SELECT_GENETIC_CODE` entirely — `MINIPROT_CDS` resolves `cds.gff`/`genetic_code.json` directly in plain shell (task 38 §3). |
...
| 13a | `ANNOTATE` | C8 (`annotate_summary.py`) + MITOS2 on `animal_mt` | `cds.gff`, target contigs | annotation GFF3 + `annotation_summary.json` | [§4.4](04-reference-data.md#44-consolidated-build-script) (MITOS2 refdata) | Supplementary organelle annotation ([brief.md §3.6](brief.md)); feeds the confidence-scoring step below and stands alone as an output. **A merge, not a second annotator:** CDS features come from stage 12 unmodified; tRNA/rRNA come from a specialist annotator where one exists. `animal_mt` → MITOS2 (22 of its 37 genes are tRNAs, so CDS-only would show a third of the mitogenome). MITOS2's *own* CDS calls are compared against the miniprot winners as an independent cross-check and normally discarded as features — except a call with no miniprot counterpart at all (`annotator_only`) is rescued into a real feature, attributed to its annotator, because miniprot systematically misses short, fast-evolving genes (`ATP8`) on under-referenced taxa (task 39). `coordinate_conflicts` stays untouched: wherever both methods called a gene, miniprot remains authoritative. Plant arms have no specialist annotator yet ([§8 item 3](07-open-questions.md#8-remaining-open-questions)) and emit `status: "ok_cds_only"` — a real annotation of every protein-coding gene with the missing feature classes stated, not implied by absence ([principle 7](../CONSTITUTION.md)). Never gates a sample: annotation counts are data for the report, and the negative-clarity classification stays centralised in `COLLATE`. |
| 13a cont'd | `TRANSLATE_ANNOTATION_CDS` → `SCORE_ANNOTATION_BLASTP` → `JOIN_ANNOTATION_SCORES` | C10 (`translate_annotation_cds.py`) + blastp + C11 (`annotation_scores.py`) | `ANNOTATE`'s merged GFF + `annotation_summary.json`, target contigs, **comprehensive** protein panel (same as stage 12) | annotation GFF3 + `annotation_summary.json`, now carrying `pident`/`qcovhsp`/`bitscore` per CDS feature | [§4.3](04-reference-data.md#43-protein-panel-and-barcode-selector) | **Confidence scoring (task 40).** Neither source tool's own score is portable: miniprot's DP alignment score isn't comparable outside miniprot and isn't even stable across settings (task 38 moved a locus's score from 776 to 1012 with identical coordinates); MITOS2's exon score is length-driven and unbounded. Bitscore is the portable statistic — a log-odds sum independent of database size, so translated CDS features are blastp'd against the same per-gene panel `MINIPROT_CDS` aligned to, and `pident`/`qcovhsp`/`bitscore` are read off the best hit. **Three processes, not one**, because the pinned BLAST biocontainer carries no Python at all (confirmed directly — `blast:2.17.0`'s image has no `python3` binary), so translation (needs biopython) and the GFF/summary join (needs JSON) can't run in the same container as blastp itself: `TRANSLATE_ANNOTATION_CDS` (shares `EXTRACT_BARCODES`'s `neoformit/daff-wf5-scripts` image) splices + translates every CDS feature uniformly from coordinates, regardless of provenance — miniprot's own `--trans` output is not used, since a rescued feature has no equivalent and two translation paths would defeat a comparable score — under the sample's actually-selected `genetic_code_cds` (task 38), producing one query FASTA per gene; `SCORE_ANNOTATION_BLASTP` (reuses `BLAST_VALIDATE`'s pinned BLAST 2.17.0 image, no new container) runs one `blastp -subject` call per gene against that gene's own panel file, so a hit is never contaminated by a homologous-but-wrong gene; `JOIN_ANNOTATION_SCORES` (shares the scripts image again) picks the best hit per feature (highest bitscore, ties on identity) and writes the final, published annotation GFF + `annotation_summary.json`'s new `cds_scores` list. **Scores are data, never a gate** (task 40 §5, mirrors task 31's stance on annotation counts): no feature is dropped, re-called, or excluded because of its score, and no sample status depends on it — a feature with no blastp hit carries an explicit `null` triplet, not a zero, and stays in the GFF. `annotation_summary.json`'s `$schema` bumps to `wf5/annotation-summary/v3`. |
| 14 | `ORGANELLE_MAP` | **TBD** (see [§8](07-open-questions.md#8-remaining-open-questions)) | GFF3 + `annotation_summary.json` from `JOIN_ANNOTATION_SCORES` | organelle map (PNG/SVG) | — | Diagnostic visualisation from the full, scored annotation. Renders a circular/linear feature map for both plant (cp, mt) and animal (mt) targets. Tool choice deferred. |
...
| C5 | `validate_barcodes.py` | `wf5/barcode-validate:<tag>` (Python + `biopython`) | [§2 stage 13](#2-stage-detail) | **Subsets** stage 12's `cds.gff` to the loci named in `assets/loci.json` (case-insensitive symbol match), then validates each: per-locus length check, ORF check under the target-appropriate NCBI genetic-code table (table 11 for `plant_pt`, table 1 for `plant_mt`, table 2/5 clade-trial for `animal_mt` — independent of, and not necessarily equal to, C9's per-sample selection below), internal-stop check, protein-identity threshold. Emits `barcodes.fasta` (validated only) + a per-locus `validation.tsv` recording pass/fail reasons for panel loci that dropped out, including `not_found`. **Does not run miniprot and does not re-derive coordinates** — it consumes stage 12's alignment, which is what keeps the barcode identical to the annotated feature. **`animal_mt` clade trial** ([§3.3](03-organelles.md#33-specific-issues-and-decisions)): attempts NCBI tables 2 (vertebrate) and 5 (invertebrate) per locus, picks the one with a valid ORF, records the chosen table in metadata. | [§3.2](03-organelles.md#32-per-stage-parameter-selection), [§3.3](03-organelles.md#33-specific-issues-and-decisions) |
| C9 | `select_genetic_code.py`, run by its own `SELECT_GENETIC_CODE` process | shares `EXTRACT_BARCODES`'s `neoformit/daff-wf5-scripts` image — the miniprot biocontainer used by `MINIPROT_CDS` itself has no Python, so this can't share that process/container (rule 14) | [§2 stage 12](#2-stage-detail), `animal_mt` only | Picks between `MINIPROT_CDS`'s per-table candidate `cds.gff` files: for each, sums the best miniprot alignment score (`mRNA` column 6) per gene across the panel, then selects the highest total, tie-broken on distinct gene count and finally on trial order. Deliberately ignores `Frameshift`/`StopCodon` flag counts — they are symptoms of the wrong table, not an independent signal (task 38 §2). Copies the winning candidate to `cds.gff` and writes `genetic_code.json` recording the winner and every candidate's score/gene count — the audit trail for a decision made under uncertainty ([rule 16](../CONSTITUTION.md)). Not invoked for single-table targets (`plant_pt`, `plant_mt`), which get `cds.gff`/`genetic_code.json` resolved directly inside `MINIPROT_CDS` (plain shell, no Python needed). | task 38 §2/§3 |
| C8 | `annotate_summary.py` | shares the `ANNOTATE` MITOS2 biocontainer (stdlib-only) | [§2 stage 13a](#2-stage-detail) | Merges `cds.gff` with a specialist annotator's non-CDS features into one GFF3 keyed by contig name; rewrites `seqid` (MITOS2 writes one output subdirectory per input record, indexed internally); cross-checks the annotator's CDS calls against miniprot's, records agreements, single-source calls and coordinate conflicts, and **acts on exactly one bucket** — `annotator_only` (task 39). A call with no miniprot counterpart at all is rescued into a real CDS feature when (1) its name canonicalises into the target's `protein_coding` set (an off-panel call like a LAGLIDADG homing endonuclease never rescues) and (2) its it doesn't occupy the same locus as a same-strand miniprot CDS, judged as overlap covering ≥ 50 % of the shorter of the two features (a position-based guard, not just a name test — a call whose name fails to canonicalise can still be a positional duplicate). The test is a fraction, not any-overlap: compact organelle genomes routinely abut neighbouring genes by a few bp — the canonical 7 bp `ATGATAA` ATP8/ATP6 junction is evidence a gene is real, not evidence of duplication — so an any-overlap guard would reject the short, fast-evolving genes the rescue exists to recover. Off-panel outranks overlap when reporting the reason a call was held back, since an off-panel call would never rescue whatever its coordinates. Calls held back either way stay in `annotator_only` with the reason; rescued genes are also listed in `cds_rescued` and count toward `protein_coding_completeness`. Fragment-suffixed MITOS2 calls (`atp8_0`/`atp8_1`) fold to one canonical name during normalisation and, where they overlap, cluster to the longest-spanning call so a fragmented gene rescues once, not twice. Clustering is keyed on canonical gene name and runs *after* the guards, both deliberately: a position-only cluster would silently collapse two different abutting genes and lose one with no record, and clustering before the overlap guard would let a colliding fragment win on length and discard a clean sibling that was rescuable. a rescued feature's CDS is assembled from its MITOS2 `exon` row(s) (which carry the reading frame the `gene` row's `.` phase doesn't), not synthesised from the `gene` row's span. `coordinate_conflicts` is unactioned — miniprot remains authoritative wherever both methods called a gene. A rescued feature's GFF `source` column reads `mitos`, a miniprot feature's reads `miniprot`, so a reader can separate the two without consulting the summary JSON; `cds_source` in the summary is a per-source feature count (`{"miniprot": n, "mitos": m}`), not a single string, once two sources can contribute. Normalises gene names for comparison against `assets/organelle_gene_sets.json`; computes protein-coding completeness; writes `annotation_summary.json`. Five statuses — `no_assembly`, `ok_cds_only`, `annotator_failed`, `ok`, `no_features`. `annotator_failed` is a non-CDS annotator that was configured and invoked but parsed to zero non-CDS features (distinct from `ok_cds_only`, where no annotator was configured at all); the annotator's exit code is recorded in `annotation_summary.json`'s `annotator_exit_code` regardless of outcome, `null` when no annotator ran (task 41). **`genetic_code_agreement`** compares two independently-derived values — the table `MINIPROT_CDS`/C9 actually selected for this sample (`genetic_code_cds`, read from `genetic_code.json`) against the table the non-CDS annotator was *configured* with (`genetic_code_annotate`, e.g. MITOS2's fixed `params.annotate.animal_mt.genetic_code = 5`). Before task 38 this compared MITOS2's configured table against a config constant that could not see C9's per-run choice, so it could only ever agree with itself; a `false` here is now a real QC signal (a vertebrate sample where MITOS2 stayed configured for invertebrate, or vice versa) — see [tasks/todo.md](../tasks/todo.md)'s `RUN_REPORT` item. MITOS2's own `--code` is deliberately left independent of C9's selection rather than following it, so the cross-check keeps its value (task 38 §4). **Never de-duplicates by gene name** (inverted repeats and plant-mt repeats are genuine — this is why fragment clustering is overlap-based, not name-based, and non-overlapping same-name loci stay separate). **Always exits 0** — annotation is supplementary and must not abort a batch. | [§8 item 3](07-open-questions.md#8-remaining-open-questions) |
| C10 | `translate_annotation_cds.py` | shares `EXTRACT_BARCODES`'s `neoformit/daff-wf5-scripts` image (Python + `biopython`) | [§2 stage 13a cont'd](#2-stage-detail) | Parses `ANNOTATE`'s merged GFF (shared `annotation_gff.py`, also used by C11 — one definition of "one CDS feature" regardless of provenance), splices each feature's CDS from the assembly via `extract_cds_seq` (reused from C5's `validate_barcodes.py`, not reimplemented — [rule 19](../CONSTITUTION.md)), and translates it whole-sequence (truncated to a full codon count) under `annotation_summary.json`'s `genetic_code_cds` — not the CIGAR-segmented path C5 uses for barcode ORF validation, since a rescued feature has no CIGAR and two translation paths would defeat a comparable score. Writes one protein FASTA per gene (`queries/<GENE>.faa`) so the next step can `blastp -subject` each gene against its own panel file. A feature that translates to nothing (empty splice) is silently absent from `queries/` — the join step still emits it with an explicit `null` score. | task 40 §2 |
| C11 | `annotation_scores.py` | shares `EXTRACT_BARCODES`'s `neoformit/daff-wf5-scripts` image | [§2 stage 13a cont'd](#2-stage-detail) | Reads `SCORE_ANNOTATION_BLASTP`'s `blast.tsv`, picks the best hit per feature (highest bitscore, ties broken on `pident`; malformed/truncated rows skipped with a warning, not fatal), and writes the scored GFF (score triplet appended to each mRNA line's attributes, `null` where there is no hit) + `annotation_summary.json`'s `cds_scores` list (`id`, `gene`, `seqid`, `start`, `end`, `source`, `pident`, `qcovhsp`, `bitscore`, `sseqid`). Never reads or reasons about the summary's `status` field and never drops a feature — scores are data, not a gate (task 40 §5). Bumps `$schema` to `wf5/annotation-summary/v3`. | task 40 §6 |
| C12 | `annotate_graph_svg.py` | shares `EXTRACT_BARCODES`'s `neoformit/daff-wf5-scripts` image — the BandageNG biocontainer has no Python, so this runs as a second script step inside `BANDAGE_NG` rather than in BandageNG's own container (rule 14, same constraint as C9) | [§2 stage 9](#2-stage-detail) | Turns BandageNG's semantically flat Qt SVG export into a node-labelled one. Generates the `name,color` sentinel CSV consumed by `BandageNG image --color` (one unique `#RRGGBB` per GFA segment, allocated deterministically from segment order so a rerun produces an identical SVG), then post-processes the result: for each `fill="#rrggbb"` matching a sentinel, replaces the fill with the neutral display colour and attaches `data-node="<segment>"` plus a `<title>` child (native browser tooltip, no JS required for the basic label). Also parses `assembly_info.txt`'s `graph_path` column to attach `data-contigs` — the contig(s) whose path traverses that segment — as a comma-separated list, because the segment↔contig relation is many-to-many (a plastid inverted-repeat edge is traversed twice, and a repeat edge can be shared between a target and an off-target contig). **Never invents a per-node bucket**: where an edge is traversed by contigs in different bins, all of them are listed and the colouring stays neutral ([principle 7](../CONSTITUTION.md)). A sentinel that matches no path (node not drawn) and a path whose fill matches no sentinel are both warnings, not errors. **Always exits 0** — the graph is a diagnostic and must not abort a sample; on any failure it leaves the unannotated SVG in place. | task 47_bandage_interactive_svg.md |
| C6 | `collate.py` | `wf5/report:<tag>` (shared with C7 and the report renderer — Python + Jinja2 per [§6a.5](06a-reports.md#6a5-report-implementation-boilerplate)) | [§2 stage 15](#2-stage-detail) | Detects the [`COVERAGE_GATE`](#21-coverage-gate-2-stage-6) soft-fail marker and dispatches to either the full or minimal per-sample bundle, aggregates upstream outputs into the layout in [§0](00-overview.md#0-input-sample-sheet), emits `metadata.json` (sample meta + tool versions + reference-bundle version + `read_qc` (task 43a §4.1) + `MINIPROT_CDS`'s `genetic_code.json` — the selected table and, on `animal_mt`, every candidate considered, per [rule 16](../CONSTITUTION.md) and task 38 §4), and invokes the report renderer (`bin/report/report.py`) to write `report.html`. | [§6a.5](06a-reports.md#6a5-report-implementation-boilerplate) |
| C7 | `run_report.py` | shares `wf5/report:<tag>` with C6 | [§2 stage 16](#2-stage-detail) | Cross-sample join: reads every per-sample `metadata.json` and its `sample_status` field — `ok` / `low_coverage` (assembled under the warn floor) / `no_assembly` (recruited but nothing assembled) / `no_barcode` (assembled but no locus extractable) / `fail` (coverage-gate soft-fail), the vocabulary of [principle 7](../CONSTITUTION.md) — and adds `error` (unexpected tool crash), the one value C7 derives itself because a crashed sample never produced a `metadata.json`. `no_assembly` and `no_barcode` stay distinct in the run summary, never collapsed into one "no recovery" bucket. Emits `run_manifest.json` (samplesheet snapshot + reference-bundle version from [§4.4 manifest](04-reference-data.md#44-consolidated-build-script) + pipeline commit + invocation timestamp), and renders the run-level HTML via the same Jinja machinery as C6. | [§6a.4](06a-reports.md#6a4-run-level-run-reporthtml) |
...
no `pandas`. C10 and C11 add **no** image either: both share C5/C9's
`neoformit/daff-wf5-scripts` image (task 40 needs the same biopython
already pinned there), and the blastp step between them reuses
`BLAST_VALIDATE`'s pinned BLAST 2.17.0 image rather than the MITOS2
image stage 13a itself runs in — the BLAST image has no Python
interpreter at all, so it could not have hosted C10/C11 even had that
been preferred (task 40 §4). Net: **five bespoke images** for custom
code, plus per-tool biocontainers for the off-the-shelf stages (see
